module InitController(
	...
	wire LSTC =
		IS==8'h0 ? LS[12:0]==13'h0000 : // POR pause
		IS==8'h1 ? LS[12:0]==13'h03FF : // Check flash programmer attached
		IS==8'h2 ? LS[12:0]==13'h0000 : // Issue flash command
		IS==8'h3 ? LS[12:0]==13'h0007 : // Load driver from flash to RAM
		IS==8'h4 ? LS[12:0]==13'h0000 : // End flash command
		IS==8'h5 ? LS[12:0]==13'h0000 : // Issue flash command
		IS==8'h6 ? LS[12:0]==13'h1FFF : // Load image from flash to RAM
		IS==8'h7 ? LS[12:0]==13'h0000 : // End flash command
		IS==8'h8 ? LS[12:0]==13'h0000 : // Operation mode
		IS==8'h9 ? LS[12:0]==13'h0000 : // Inhibit mode
		1; // Other
	always @(posedge CLK) begin
		if (CSTC) begin
			if (LSTC) LS <= 0;
	...
	always @(posedge CLK) RAMAddr[24:0] <=
		...
		IS==6 ? {  RAMImageAddr[24:23], LS[12:0], CS[11:2] } : 0;
	...
	/* Flash address */
	wire [23:0] FlashAddr = 
		(IS==2) ? FlashDriverAddr[23:0] : FlashImageAddr[23:0];
		...
		case (IS)
			0, 1: begin // POR pause and flash check
				...
				case (CS[12:0]) // Send command
					13'h1010, 13'h1011, 13'h1012, 13'h1013,
				...
				case (CS)
					...
					13'h1006: RAMCmd <= `RC_LDM;
					13'h100B,
					13'h1011, 13'h1016, 13'h101B,
					13'h1021, 13'h1026, 13'h102B,
					13'h1031, 13'h1036, 13'h103B: RAMCmd <= `RC_Ref;
	...
	always @(posedge CLK) begin
		WRD[7:0] <= SetRestoreEN ? { MOSIr, MISOr, WRD[7:2] } : 8'h00;
	end
